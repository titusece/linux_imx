...
static char leonardo_fps_list[leonardo_max_fps+1] =
{
[leonardo_60_fps] = 60,
[leonardo_30_fps] = 30,
[leonardo_45_fps] = 45,
...
struct leonardo_mode_info {
	const char *name;
	enum leonardo_mode mode;
	u32 width;
	u32 height;
	u32 min_width;
	u32 min_height;
...
static const struct leonardo_mode_info leonardo_mode_info_data[leonardo_max_fps][leonardo_mode_MAX] = {
	[leonardo_30_fps][leonardo_mode_SVGA_800_600] =
		{"800x600@30", leonardo_mode_SVGA_800_600, 800, 600, 8, 8, 4096, 4096, 8, 8,
		MIPI_DT_RGB888,
		},
	[leonardo_45_fps][leonardo_mode_SVGA_800_600] =
		{"800x600@45", leonardo_mode_SVGA_800_600,  800, 600, 8, 8, 4096, 4096, 8, 8,
		MIPI_DT_RGB888,
		},
	[leonardo_60_fps][leonardo_mode_SVGA_800_600] =
		{"800x600@60", leonardo_mode_SVGA_800_600,  800, 600, 8, 8, 4096, 4096, 8, 8,
		MIPI_DT_RGB888,
		},
};
...
static int ioctl_bcrm_i2cwrite_reg( struct v4l2_control *vc, unsigned int reg, int length)
{
	...
	LNT_TRACE("reg %x, length %d, vc->value %u, vc->value 0x%x\n",reg,length,vc->value,vc->value);
...
//	memcpy(i2c_reg.buffer, &vc->value, sizeof(vc->value) );
	...
	LNT_TRACE("Before i2c_reg.reg %x, i2c_reg.count %d, vc->value %u\n",i2c_reg.reg,i2c_reg.count,vc->value);

//        status = bcrm_i2cwrite_reg(&i2c_reg);
	ioctl_gencam_i2cwrite_reg(&s, &i2c_reg);


	LNT_TRACE("After i2c_reg.reg %x, i2c_reg.count %d, vc->value %u\n",i2c_reg.reg,i2c_reg.count,vc->value);
...
{
	LNT_TRACE("START, fsize->index %d\n",fsize->index);

	if (fsize->index > leonardo_mode_MAX)
		return -EINVAL;

	fsize->pixel_format = leonardo_data.pix.pixelformat;
	fsize->type = V4L2_FRMSIZE_TYPE_STEPWISE;
	fsize->stepwise.step_width =
			leonardo_mode_info_data[0][fsize->index].step_width;
	fsize->stepwise.step_height =
			    leonardo_mode_info_data[0][fsize->index].step_height;

	fsize->stepwise.min_width =
			leonardo_mode_info_data[0][fsize->index].min_width;
	fsize->stepwise.min_height =
			    leonardo_mode_info_data[0][fsize->index].min_height;

	fsize->stepwise.max_width =
			leonardo_mode_info_data[0][fsize->index].max_width;
	fsize->stepwise.max_height =
			    leonardo_mode_info_data[0][fsize->index].max_height;


	LNT_TRACE("leonardo_mode_info_data[0][fsize->index].width %d \n",leonardo_mode_info_data[0][fsize->index].width);
	LNT_TRACE("leonardo_mode_info_data[0][fsize->index].height %d\n",leonardo_mode_info_data[0][fsize->index].height);
					 ...
					 struct v4l2_frmivalenum *fival)
{
	...
	for (i = 0; i < ARRAY_SIZE(leonardo_mode_info_data); i++)
		for (j = 0; j < (leonardo_mode_MAX + 1); j++)
			if (fival->pixel_format == leonardo_data.pix.pixelformat
			 && fival->width == leonardo_mode_info_data[i][j].width
			 && fival->height == leonardo_mode_info_data[i][j].height
			 && fival->index == count++) {
...
//						leonardo_fps_list[i];
			...
			LNT_TRACE("fival->pixel_format %u, fival->width %d, fival->height %d, fival->index %d, leonardo_fps_list[%d] %d\n",fival->pixel_format,fival->width,fival->height,fival->index,i,leonardo_fps_list[i]);
			LNT_TRACE("END\n");
				return 0;
			}
	LNT_TRACE("END\n");
...
//	u32 value_hex;
	...
	case V4L2_CID_EXPOSURE: {
		...
		/* Reading the Current Exposure time */
        	...
        	i2c_reg.count = 8;//8Bytes
        	i2c_reg.buffer = &value;
	
		status = i2c_read(&i2c_reg);//TODO
//		LNT_TRACE("Current Before SWAP Exposure value : 0x%lx, 0x%d\n",value,value_hex);
		LNT_TRACE("Current Before SWAP Exposure value : %u\n",value);
		swapbytes(&value, 8);
...
//		LNT_TRACE("Current Exposure value : 0x%lx, %d\n",value,value_hex);
		LNT_TRACE("Current Exposure value : %u\n",value);
		...
		status = i2c_read(&i2c_reg);//TODO
		LNT_TRACE("Exposure Before SWAP Minimum value(DEC) : %u, qctrl->minimum(HEX) : 0x%x\n",value,qctrl->minimum);
		swapbytes(&value, 8);
		qctrl->minimum = value;
		LNT_TRACE("Exposure Minimum value(DEC) : %u, qctrl->minimum(HEX) : 0x%x,\n",value,qctrl->minimum);
		...
		status = i2c_read(&i2c_reg);//TODO
		LNT_TRACE("Exposure Before SWAP Maximum value(DEC) : %u, qctrl->maximum(HEX) : 0x%x\n",value,qctrl->maximum);
		swapbytes(&value, 8);
		qctrl->maximum = value;
		LNT_TRACE("Exposure Maximum value(DEC) : %u, qctrl->maximum(HEX) : 0x%x\n",value,qctrl->maximum);
		...
		status = i2c_read(&i2c_reg);//TODO
		LNT_TRACE("Exposure Before SWAP Step value(DEC) : %u, qctrl->step(HEX) : 0x%x\n",value,qctrl->step);
		swapbytes(&value, 8);
		qctrl->step = value;
		LNT_TRACE("Exposure Step value(DEC) : %u, qctrl->step(HEX) : 0x%x\n",value,qctrl->step);
...
//		qctrl->type = V4L2_CTRL_TYPE_INTEGER64;//64bit value
	...
	case V4L2_CID_EXPOSURE_AUTO: {
		...
		/* Reading the Current Exposure Auto */
	        i2c_reg.reg = cci_reg.BCRM_Address + EXPOSURE_AUTO_8RW;
        	i2c_reg.reg_size = 2;//2 Bytes address size
        	i2c_reg.count = 1;//1Byte
        	i2c_reg.buffer = &value;
	
		status = i2c_read(&i2c_reg);//TODO
		LNT_TRACE("Current Exposure Auto value : %u\n",value);

		//BCRM Auto Gain/Exposure changes
		if(value == 2)
		qctrl->default_value = true;//true (ON) for continous mode //Refer BCRM doc
		else
		qctrl->default_value = false;//false (OFF) for off & once mode //Refer BCRM doc


		qctrl->minimum = 0;//Refer BCRM doc
		qctrl->step = 1;
		qctrl->maximum = 2;
		LNT_TRACE("Exposure Auto Minimum value (DEC) : %u, qctrl->minimum (HEX) : 0x%x\n",value,qctrl->minimum);
		LNT_TRACE("Exposure Auto Maximum value (DEC) : %u, qctrl->maximum (HEX): 0x%x\n",value,qctrl->maximum);
		LNT_TRACE("Exposure Auto Step value (DEC) : %u, qctrl->step (HEX) : 0x%x\n",value,qctrl->step);
	...
	case V4L2_CID_GAIN: {
		...
		status = i2c_read(&i2c_reg);//TODO
		LNT_TRACE("Current Before SWAP Gain value : %u\n",value);
		swapbytes(&value, 8);
//		float_val = *(float *)&value;//TODO Build error
//		LNT_TRACE("Current Gain value : 0x%lx, %f\n",value,float_val);
		LNT_TRACE("Current Gain value : %u\n",value);
		qctrl->default_value = value;
		...
		status = i2c_read(&i2c_reg);//TODO
		LNT_TRACE("Before SWAP Minimum Gain value %u\n",value);
		swapbytes(&value, 8);
//		value >>= 32;//TODO Its required if we use 64bits
//		LNT_TRACE("After shift value : %llx, %lx\n",value,value);
		qctrl->minimum = value;
		LNT_TRACE("qctrl->minimum (DEC) : %u, Minimum Gain value (HEX) 0x%x\n",qctrl->minimum,value);
		...
		status = i2c_read(&i2c_reg);
		LNT_TRACE("Before SWAP Maximum Gain value %u\n",value);
		swapbytes(&value, 8);
		qctrl->maximum = value;
		LNT_TRACE("qctrl->maximum : %u, Maximum Gain value (HEX) 0x%x\n",qctrl->maximum,value);
		...
		status = i2c_read(&i2c_reg);//TODO
		LNT_TRACE("Before SWAP Gain Step value(HEX) 0x%x \n",value);
		swapbytes(&value, 8);
		qctrl->step = value;
		LNT_TRACE("qctrl->step : %u, Gain Step value(HEX) 0x%x \n",qctrl->step,value);
...
//		qctrl->type = V4L2_CTRL_TYPE_INTEGER64;//64bit value
	...
	case V4L2_CID_AUTOGAIN: {
		...
		/* Reading the Current Auto Gain */
	        i2c_reg.reg = cci_reg.BCRM_Address + GAIN_AUTO_8RW;
        	i2c_reg.reg_size = 2;//2 Bytes address size
        	i2c_reg.count = 1;//1Byte
        	i2c_reg.buffer = &value;
	
		status = i2c_read(&i2c_reg);//TODO
		LNT_TRACE("Current Gain Auto value : %u\n",value);
		...
		qctrl->step = 1;
		qctrl->maximum = 2;
		LNT_TRACE("Gain Auto Minimum value : %u, qctrl->minimum(HEX) : 0x%x\n",value,qctrl->minimum);
		LNT_TRACE("Gain Auto Maximum value : %u, qctrl->maximum(HEX) : 0x%x\n",value,qctrl->maximum);
		LNT_TRACE("Gain Auto Step value : %u, qctrl->step(HEX) : 0x%x\n",value,qctrl->step);
...
//		vc->value = leonardo_data.ae_mode;
	...
	LNT_TRACE("Before SWAP vc->value %u \n",vc->value);
	swapbytes(&vc->value, i2c_reg.count);
	LNT_TRACE("After SWAP vc->value %u \n",vc->value);

//BCRM Auto Gain/Exposure changes
	if(vc->id == V4L2_CID_EXPOSURE_AUTO || vc->id == V4L2_CID_AUTOGAIN) {
	
		LNT_TRACE("------> Any one of the option V4L2_CID_EXPOSURE_AUTO & V4L2_CID_AUTOGAIN is selected. vc->value %d\n",vc->value);
		if(vc->value == 2) {
		vc->value = true;//true (ON) for continous mode //Refer BCRM doc
		LNT_TRACE("------> IF vc->value %u\n",vc->value);
		}
		else {
		LNT_TRACE("------> ELSE  vc->value %u\n",vc->value);
	...
	LNT_TRACE("vc->value %u, value 0x%x \n",vc->value,vc->value);
...
//LnT
	case V4L2_CID_EXPOSURE:
		LNT_TRACE("V4L2_CID_EXPOSURE, cci_reg.BCRM_Address 0x%x, vc->value %u\n",cci_reg.BCRM_Address,vc->value);
		reg = EXPOSURE_TIME_64RW; 
		length = 8;

		qctrl.id = V4L2_CID_EXPOSURE;
		ioctl_queryctrl(s, &qctrl);
		LNT_TRACE("V4L2_CID_EXPOSURE, qctrl.minimum %u, qctrl.maximum %u\n",qctrl.minimum,qctrl.maximum);

		if(vc->value <= qctrl.minimum) {
			LNT_TRACE("vc->value is less than or equal to minimum; vc->value %u, qctrl.minimum %u\n",vc->value,qctrl.minimum);
			vc->value = qctrl.minimum;
		}
		else if(vc->value >= qctrl.maximum) {
			LNT_TRACE("vc->value is greater than or equal to maximum; vc->value %u, qctrl.maximum %u\n",vc->value,qctrl.maximum);
			vc->value = qctrl.maximum;
		}

		LNT_TRACE("after V4L2_CID_EXPOSURE, vc->value %u\n",vc->value);

		break;
	case V4L2_CID_EXPOSURE_AUTO:
		LNT_TRACE("V4L2_CID_EXPOSURE_AUTO vc->value %u\n",vc->value);
		reg = EXPOSURE_AUTO_8RW; 
		length = 1;
		...
		LNT_TRACE("after V4L2_CID_EXPOSURE_AUTO, vc->value %u\n",vc->value);

		break;
	case V4L2_CID_AUTOGAIN:
		LNT_TRACE("V4L2_CID_AUTOGAIN vc->value %u\n",vc->value);
		reg = GAIN_AUTO_8RW;
		length = 1;
		...
		LNT_TRACE("after V4L2_CID_AUTOGAIN, vc->value %u\n",vc->value);

	        break;
	case V4L2_CID_GAIN:
		LNT_TRACE("V4L2_CID_GAIN, vc->value %u\n",vc->value);
		reg = GAIN_64RW; 
		length = 8;

		qctrl.id = V4L2_CID_GAIN;
		ioctl_queryctrl(s, &qctrl);
		LNT_TRACE("V4L2_CID_GAIN, qctrl.minimum %u, qctrl.maximum %u\n",qctrl.minimum,qctrl.maximum);

		if(qctrl.minimum >= vc->value)
			vc->value = qctrl.minimum;

		if(qctrl.maximum <= vc->value)
			vc->value = qctrl.maximum;

		LNT_TRACE("after V4L2_CID_GAIN, vc->value %u\n",vc->value);
		break;
...
//				mipi_csi2_reset_with_dphy_freq(mipi_csi2_info, 0x44);//187.5MHz <-> 1.5Gbps (1500Mbps)
				mipi_csi2_reset_with_dphy_freq(mipi_csi2_info, 0x24);//87.5MHz <-> 0.7Gbps (700Mbps); 87.5*2 -> 175MHz -> 
//TODO
				pr_err("mipi_csi2_reset_with_dphy_freq is configured with 0x24 for 700Mbps configuration.\n");
