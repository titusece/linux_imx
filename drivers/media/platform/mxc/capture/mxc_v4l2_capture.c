...
/* Required for Eventfd */
pid_t pid = 0;
unsigned int pid_val;
unsigned int efd_val;
struct task_struct * userspace_task = NULL; // userspace program's task struct
struct file * efd_file = NULL;          //eventfd's file struct
struct eventfd_ctx * efd_ctx = NULL;        //eventfd context
static unsigned int eventfd_val = 0;
static int eventfd_func(int val);

static char leonardo_video_name[20];
...
/* LnT */
static int count_mem_allo = 0;
static int count_mem_clear = 0;
...
static int mxc_free_frame_buf(cam_data *cam)
{
	...
	pr_debug("%s, count_mem_allo %d\n", __func__,count_mem_allo);

/* LnT  */
	for (i = 0; i < FRAME_NUM; i++) {
		pr_info(" mxc_free_frame_buf, FRAME_NO, i %d, cam->frame[%d].vaddress %u\n",i,i,cam->frame[i].vaddress);
...
static void streamoff_work_handler(struct work_struct *w)
{
	...
	if (!list_empty(&cam->working_q)) {
		LNT_TRACE("WORKING queue is not empty so there are partial filled frames.\n");
		incomplete_count++;
		...
		LNT_TRACE("list_for_each_entry, dummy_frame_buf->vaddress 0x%x, dummy_frame_buf->buffer.m.offset 0x%x, dummy_frame_buf->buffer.index 0x%x, dummy_frame_buf->buffer.flags 0x%x\n",dummy_frame_buf->vaddress,dummy_frame_buf->buffer.m.offset,dummy_frame_buf->buffer.index,dummy_frame_buf->buffer.flags);
		...
		msleep(1000 * 1);//1Sec
			if(list_empty(&cam->working_q)) {
				printk("%s:%d incomplete frame processing done!\n",__func__,__LINE__);//TODO need to remove
				goto exit;
			}
		}
	}
...
exit:
	...
	LNT_TRACE("list_for_each_entry is done for cam->working_q and streamoff_done is set., Total no of incomplete frames : %d \n",incomplete_count);
...
static void flush_work_handler(struct work_struct *w)
{
	...
	int err;
	static int incomplete_count = 0;
	static int unused_count = 0;

	pr_debug("%s:%d\n",__func__,__LINE__);

	if( list_empty(&cam->working_q) && list_empty(&cam->ready_q) ) {
		LNT_TRACE("WORKING queue is empty so there are no partial filled frames or incomplete frames\n");
		goto exit;		
	}
	...
	if (!list_empty(&cam->working_q)) {
		...
		msleep(1000 * 1);//1Sec
			if(list_empty(&cam->working_q)) {
				LNT_TRACE("%s:%d incomplete frame processing done!\n",__func__,__LINE__);//TODO need to remove
				goto exit;
			}
		}
	}
	...
	if (!list_empty(&cam->ready_q)) {
		LNT_TRACE("%s:%d, READY queue is not empty so there are unused frames.\n",__func__,__LINE__);
		unused_count++;
		...
		msleep(1000 * 1);//1Sec
			if(list_empty(&cam->ready_q)) {
				LNT_TRACE("%s:%d unused frame processing done!\n",__func__,__LINE__);
				goto exit;
			}
		}
	}
...
exit:
	g_flush_frames_status = flush_frames_incomplete_done;//need again for exit case.
	g_flush_frames_status = flush_frames_unused_done;

	LNT_TRACE("list_for_each_entry is done forcam->working_q & cam->ready_q,  Total no of incomplete frames : %d, Total no of unused frames : %d\n",incomplete_count,unused_count);
...
//TODO
	...
	if (cam->enc_enable_csi) {
		LNT_TRACE("cam->enc_enable_csi called again after flush.\n");
		err = cam->enc_enable_csi(cam);
		if (err != 0) {
			pr_err("Error : cam->enc_enable_csi called again after flush.\n");
			return;
		}
	}
#endif
...
static int v4l2_dma_alloc_mem(cam_data *cam, struct v4l2_dma_mem *dma_mem)
{
	...
	{
	count_mem_clear++;
	pr_info("vidioc_int_dma_alloc_mem: Freeing frames for only one time count_mem_clear %d\n",count_mem_clear);
	...
	pr_debug("%s: size=%d\n", __func__, cam->v2f.fmt.pix.sizeimage);
		...
		cam->frame[dma_mem->index].buffer.length =
		    PAGE_ALIGN(cam->v2f.fmt.pix.sizeimage);
	...
	pr_info("VIDIOC_MEM_ALLOC count_mem_allo is incremented to %d , cam->frame[%d].vaddress 0x%x  dma_mem->index  %d  \n",count_mem_allo,count_mem_allo,cam->frame[dma_mem->index].vaddress,dma_mem->index);
...
static int v4l2_dma_alloc_mem_free(cam_data *cam, struct v4l2_dma_mem *dma_mem)
{
	...
	if (dma_mem->index < 0 || FRAME_NUM < dma_mem->index )
	{
	pr_info("VIDIOC_MEM_FREE: Please do make sure the frame no which you want to free! \n");
	return -EINVAL;
	}

		if (cam->frame[dma_mem->index].vaddress != 0) {
		pr_info("VIDIOC_MEM_FREE: FRAME_NO %d freed, cam->frame[%d].vaddress 0x%x\n",dma_mem->index,dma_mem->index,cam->frame[dma_mem->index].vaddress);
...
static int mxc_allocate_frame_buf(cam_data *cam, int count)
{
	...
	pr_info("VIDIOC_REQBUFS count is incremented to %d , cam->frame[%d].vaddress 0x%x \n",i,i,cam->frame[i].vaddress);
...
/* LnT : Debug */
	mipi_csi2_reg_dump(mipi_csi2_info);
#endif
...
//TODO: We can also QUEUE after STREAMON
...
//	if(cam->queued) {
//		pr_debug("%s:%d PROBE1 QUEUED already before STREAMON\n", __func__,__LINE__);
...
static int mxc_v4l2_g_ctrl(cam_data *cam, struct v4l2_control *c)
{
	...
	case V4L2_CID_EXPOSURE_AUTO:
		if (cam->sensor) {
			c->value = cam->expos_auto;
			LNT_TRACE("c->value %d\n",c->value);
			status = vidioc_int_g_ctrl(cam->sensor, c);
			LNT_TRACE("c->value %d\n",c->value);
			cam->expos_auto = c->value;
		} else {
			pr_err("ERROR: v4l2 capture: slave not found!\n");
			status = -ENODEV;
		}
	...
	case V4L2_CID_AUTOGAIN:
		if (cam->sensor) {
			c->value = cam->gain_auto;
			LNT_TRACE("c->value %d\n",c->value);
			status = vidioc_int_g_ctrl(cam->sensor, c);
			LNT_TRACE("c->value %d\n",c->value);
			cam->gain_auto = c->value;
		} else {
			pr_err("ERROR: v4l2 capture: slave not found!\n");
			status = -ENODEV;
		}
...
static int mxc_v4l2_query_ctrl(cam_data *cam,
	...
	if (vidioc_int_queryctrl(cam->sensor, qctrl)) {
		ret = -EINVAL;
	}

	pr_debug("###########  %s done, qctrl->minimum 0x%x, qctrl->maximum 0x%x ########### \n",__func__,qctrl->minimum,qctrl->maximum);
...
static int mxc_v4l_flush_frames(cam_data *cam, struct v4l2_buffer *buf)
{
	...
	pr_debug("%s:%d cam->working_q & cam->ready_q are empty, thus exiting...\n",__func__,__LINE__);
	g_flush_status = flush_done;
	g_streamoff_status = streamoff_done;
	return 0;
	}

	schedule_delayed_work(&cam->flush_work, (HZ * 2) );
...
static int mxc_v4l_dqueue(cam_data *cam, struct v4l2_buffer *buf)
{
	...
	if (!wait_event_interruptible_timeout(cam->enc_queue,
					      cam->enc_counter != 0,
					      10 * HZ)) {//2 * HZ
		pr_err("ERROR: v4l2 capture: mxc_v4l_dqueue timeout or no frames to DQUEUE "
			"enc_counter %x\n",
		       cam->enc_counter);
		return -ETIME;
	...
	LNT_TRACE("g_flush_status %d, g_streamoff_status %d, g_flush_frames_status %d, g_streamoff_frames_status %d\n",g_flush_status,g_streamoff_status,g_flush_frames_status,g_streamoff_frames_status);


	if(g_streamoff_frames_status == streamoff_frames_complete) {
		LNT_TRACE("DQUEUEing the complete frame (NA)\n");
		...
		LNT_TRACE("DQUEUEing the unused frame, flush\n");
		...
		frames_count.incomplete_frames_count++;
		LNT_TRACE("DQUEUEing the incomplete frame, streamoff\n");
		frame = list_entry(cam->working_q.next, struct mxc_v4l_frame, queue);
		list_del(cam->working_q.next);
		list_add_tail(&frame->queue, &cam->done_q);
		...
		frames_count.complete_frames_count++;
		LNT_TRACE("DQUEUEing the complete frame\n");
		frame = list_entry(cam->done_q.next, struct mxc_v4l_frame, queue);
		list_del(cam->done_q.next);
	}
...
static void power_down_callback(struct work_struct *work)
{
	...
	LNT_TRACE("power_down_callback: called!\n");

	cam_data *cam = container_of(work, struct _cam_data, power_down_work.work);

	LNT_TRACE("power_down_callback: After container_of\n");
...
{
	LNT_TRACE("power_off_camera: Before cam->power_down_work\n");
	schedule_delayed_work(&cam->power_down_work, (HZ * 2));
	LNT_TRACE("power_off_camera: After cam->power_down_work\n");
}
...
static int mxc_v4l_stream_stat(cam_data *cam, struct v4l2_stream_stat *stream_stat)
{
	struct v4l2_streamparm currentparm;
	u32 current_fps;// parm_fps;
	int err = 0;
	...
	stream_stat->current_frame_rate_fps = current_fps;
	...
	pr_debug("Current framerate is %lld, stream_stat->current_frame_rate_fps %lld\n",current_fps,stream_stat->current_frame_rate_fps);
...
/* LnT */
	...
	case VIDIOC_R_I2C: {
		...
		pr_debug("   case VIDIOC_R_I2C\n");
		retval = vidioc_int_gencam_i2cread_reg(cam->sensor, i2c_reg);
		pr_debug("  case VIDIOC_R_I2C: Return i2c read value %d\n",retval);
		break;
	}
...
/* LnT */
	...
	case VIDIOC_W_I2C: {
		struct v4l2_i2c *i2c_reg = arg;
		pr_debug("   case VIDIOC_W_I2C\n");
		retval = vidioc_int_gencam_i2cwrite_reg(cam->sensor, i2c_reg);
		pr_debug("  case VIDIOC_W_I2C: Return value %d\n",retval);
		break;
	}
...
/* LnT */
	...
	case VIDIOC_MEM_ALLOC: {
		...
		pr_debug("   case VIDIOC_MEM_ALLOC\n");

		if ((dma_mem->type != V4L2_BUF_TYPE_VIDEO_CAPTURE)) {
			pr_err("ERROR: v4l2 capture: VIDIOC_MEM_ALLOC: "
			       "wrong buffer type\n");
			retval = -EINVAL;
			break;
		}
		...
		pr_debug("  case VIDIOC_MEM_ALLOC: Return value %d\n",retval);
		}
...
/* LnT */
	...
	case VIDIOC_MEM_FREE: {
		struct v4l2_dma_mem *dma_mem = arg;
		pr_debug("   case VIDIOC_MEM_FREE\n");


		if ((dma_mem->type != V4L2_BUF_TYPE_VIDEO_CAPTURE)) {
			pr_err("ERROR: v4l2 capture: VIDIOC_MEM_FREE: "
			       "wrong buffer type\n");
			retval = -EINVAL;
			break;
		}
		...
		pr_debug("  case VIDIOC_MEM_FREE: Return value %d\n",retval);
		}
...
/* LnT */
	...
	case VIDIOC_FLUSH_FRAMES: {
		...
		unsigned long lock_flags;
		pr_debug("  case VIDIOC_FLUSH_FRAMES\n");

		struct v4l2_buffer *buf = arg;
		int err;

		if(g_flush_status == flush_inprogress) {
		pr_err("VIDIOC_FLUSH_FRAMES ioctl is in progress...\n");
		break;
		}

		g_flush_status = flush_inprogress;

//TODO: Need to reverse (enable the CSI) once flush frames done.
		LNT_TRACE("%s: ipu%d/csi%d capture_on=%d %s\n", __func__, cam->ipu_id,
				cam->csi, cam->capture_on,
				mxc_capture_inputs[cam->current_input].name);

		LNT_TRACE("Cleared IPU interrupt, disable the DMA & CSI %s\n", __func__);
		...
		if ((buf->type != V4L2_BUF_TYPE_VIDEO_CAPTURE)) {
			pr_err("ERROR: v4l2 capture: VIDIOC_FLUSH_FRAMES: "
			       "wrong buffer type\n");
			retval = -EINVAL;
			break;
		...
		LNT_TRACE("case VIDIOC_FLUSH_FRAMES: Return value %d\n",retval);
		break;
	}
...
/* LnT */
	...
	case VIDIOC_STREAM_STAT: {
		struct v4l2_stream_stat *stream_statistics = arg;
		pr_debug("   case VIDIOC_STREAM_STAT\n");
		retval = mxc_v4l_stream_stat(cam, stream_statistics);
		pr_debug("  case VIDIOC_STREAM_STAT: Return value %lld, stream_statistics->current_frame_rate_fps %lld\n",retval,stream_statistics->current_frame_rate_fps);
		break;
	}
	...
	case VIDIOC_QUERYCAP: {
		...
		pr_debug("  case VIDIOC_QUERYCAP %d\n",VIDIOC_QUERYCAP);
...
//		struct mxc_v4l_frame *frame;
		int index = buf->index;
		pr_debug("   case VIDIOC_QBUF, length=%d, cam->capture_on %d, cam->queued %d\n", buf->length,cam->capture_on,cam->queued);
		...
		if(!cam->queued) {
			...
			pr_debug("   case VIDIOC_QBUF: Buffers not queued yet.\n");
			...
			pr_debug("%s:%d PROBE3 \n",__func__,__LINE__);
				...
				//list_add_tail(&frame->queue, &cam->working_q);//TODO: Kernel PANIC

				pr_debug("%s:%d PROBE10 \n",__func__,__LINE__);

				frame->ipu_buf_num = cam->ping_pong_csi;

				pr_debug("%s:%d PROBE11 \n",__func__,__LINE__);

				err |= cam->enc_update_eba(cam, frame->buffer.m.offset);

				pr_debug("%s:%d PROBE12 \n",__func__,__LINE__);

//				spin_unlock_irqrestore(&cam->queue_int_lock, lock_flags);

				pr_debug("%s:%d PROBE13 \n",__func__,__LINE__);
			} else {
				pr_debug("%s:%d PROBE14 \n",__func__,__LINE__);
//				spin_unlock_irqrestore(&cam->queue_int_lock, lock_flags);
				return -EINVAL;
			}
		...
		pr_debug("%s:%d PROBE17 \n",__func__,__LINE__);

//		spin_lock_irqsave(&cam->queue_int_lock, lock_flags);
/* LnT: TODO */
/*
		if (cam->frame[index].vaddress == 0)
			{
			pr_err("ERROR: v4l2 capture: VIDIOC_QBUF: Invalid virtual address.\n");
			retval = -EINVAL;
			}
*/

//		else 
		pr_debug("%s:%d PROBE18 \n",__func__,__LINE__);
#endif
...
if ((cam->frame[index].buffer.flags & 0x7) ==
		...
		pr_debug("   case VIDIOC_QBUF done.\n");
		break;
	}
	...
	case VIDIOC_STREAMOFF: {
		...
		pr_debug("case VIDIOC_STREAMOFF, cam->enc_counter %d\n",cam->enc_counter);
...
/*		if(g_cam_cbf_status == cam_cbf_inprogress) {
		LNT_TRACE("############## camera_callback fn is in progress... waiting till it complete... ##############\n");
		msleep(5);
		}*/
		...
		mxc_capture_inputs[cam->current_input].status |= V4L2_IN_ST_NO_POWER;
		cam->capture_on = false;
		LNT_TRACE("Cleared the IPU interrupt and disabled the CSI.\n");
		if(g_streamoff_status == streamoff_inprogress) {
		pr_err("STREAMOFF is already in progress...\n");
		break;
		}
...
//When multiple interrupts called (even we requested single frame)
		if(cam->enc_counter != 0) {
		LNT_TRACE("\"Interrupt was raised and DQUEUE is not done, so there is complete frame in done_q which is not DQUEUEd yet.\"\n");
		g_streamoff_frames_status = streamoff_frames_complete;
		}

		schedule_delayed_work(&cam->streamoff_work, (HZ * 2) );
...
//LnT
	case VIDIOC_QUERYCTRL: {
		...
		pr_debug("###########  after case VIDIOC_QUERYCTRL, retval %d, qctrl->minimum 0x%x, qctrl->maximum 0x%x, qctrl->step 0x%x ########### \n",retval,qctrl->minimum,qctrl->maximum,qctrl->step);
		break;
	}
...
static void camera_callback(u32 mask, void *dev)
{
	...
	LNT_TRACE(" camera_callback called!, cam->enc_counter %d\n",cam->enc_counter);
...
next:
	...
	eventfd_func(0);//Frame is filled, notify to user space to start the DQUEUE
...
{
	int test;
	if(efd_val == 0 || pid_val == 0)
	goto exit;

	LNT_TRACE("eventfd_func: Sent eventfd event to userspace! pid=%d efd=%d\n",pid_val,efd_val);
	pid = pid_val;
	userspace_task = pid_task(find_vpid(pid), PIDTYPE_PID);
	//pr_debug("Resolved pointer to the userspace program's task struct: %p\n",userspace_task);

	//pr_debug("Resolved pointer to the userspace program's files struct: %p\n",userspace_task->files);

	rcu_read_lock();
	efd_file = fcheck_files(userspace_task->files, efd_val);
	rcu_read_unlock();
	//pr_debug("Resolved pointer to the userspace program's eventfd's file struct: %p\n",efd_file);


	efd_ctx = eventfd_ctx_fileget(efd_file);
	if (!efd_ctx) {
		pr_debug("eventfd_ctx_fileget()\n");
		return -1;
	}
	//pr_debug("Resolved pointer to the userspace program's eventfd's context: %p\n",efd_ctx);
//	eventfd_signal(efd_ctx, eventfd_val);//Sending the video instance no to userspace
//	eventfd_signal(efd_ctx, val);//Sending the video instance no to userspace

	test = eventfd_val | val << 2;

	eventfd_signal(efd_ctx, eventfd_val | val << 2 );//Sending the video instance no to userspace
...
//LNT
	...
	*(cam->video_dev) = mxc_v4l_template;
	...
	init_waitqueue_head(&cam->flush_queue);//LNT
...
//	cam->v2f.fmt.pix.pixelformat = V4L2_PIX_FMT_RGB24;
	cam->v2f.fmt.pix.pixelformat = V4L2_PIX_FMT_CUSTOM;
...
static ssize_t show_efd(struct device *dev,
			struct device_attribute *attr, char *buf)
{
        return sprintf(buf, "%d\n", efd_val);
}
...
static ssize_t store_efd(struct device *dev,
			struct device_attribute *attr, char *buf, size_t count)
{
	sscanf(buf, "%d",&efd_val);
	return count;
}

static ssize_t store_pid(struct device *dev,
			struct device_attribute *attr, char *buf, size_t count)
{
...
static DEVICE_ATTR(efd, S_IRUGO | S_IWUGO, show_efd, store_efd);
static DEVICE_ATTR(pid, S_IRUGO | S_IWUGO, show_pid, store_pid);
...
static ssize_t leonardo_camera_name(struct device *dev,
		struct device_attribute *attr, char *buf)
{
        return sprintf(buf, "%s\n",leonardo_video_name);
}

static ssize_t leonardo_bus_info(struct device *dev,
		struct device_attribute *attr, char *buf)
{
        return sprintf(buf, "%d:%x\n",i2c_busno,i2c_address);
}


static DEVICE_ATTR(cam_name, S_IRUGO, leonardo_camera_name, NULL);
static DEVICE_ATTR(bus_info, S_IRUGO, leonardo_bus_info, NULL);
...
//LnT
		eventfd_val = slave->u.slave->master->name[(strlen(slave->u.slave->master->name)-1)] - '0';

		if( (strcmp(slave->name, "leonardo_mipi")) == 0)
		{
		strcpy(leonardo_video_name, slave->name);//leonardo_video_name = slave->name;
		pr_info("Leonardo camera is detected!, %s, slave->name = %s,  MXC is attached to /dev/video%d \n",leonardo_video_name, slave->name, eventfd_val);
		}
	...
	printk("Leonardo: i2c_address %x, i2c_busno %d \n",i2c_address, i2c_busno);
